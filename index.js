...
const start = () => {
    bot.on('message', async msg => {
        ...
        if (text === '/start') {
            if (!verifiedUsers[userId]) { // *если ещё не проверен — кидаем капчу
    ...
    // *если уже проверен пускаем дальше без капчи
    bot.sendMessage(chatId, '✅ Ты уже проверен, можешь работать дальше.И если ндао меню, глянь /help')
        }
            ...
            case userCaptcha[userId]:
                if (text === userCaptcha[userId]) {
                    console.log(userCaptcha[userId])
                     verifiedUsers[userId] = true // запомнили, что чел прошёл капчу
                    bot.sendMessage(chatId, '✅Проверка пройдена сыр ебанный, теперь ты можешь пройти дальше по той же команде /start', //ДОБАВЬ БЛЯЯЯЯЯЯЯЯЯЯЯЯЯЯЯ
